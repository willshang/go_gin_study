...
type User struct {
	...
	CreateAt time.Time `json:"create_at" orm:"column(create_at);" description:"创建时间"`
}
...
func (u *User) Del(idSlice []string) error  {
...
func (u *User) PageList(name string, pageNo int, pageSize int) ([]*User,int64, error)  {
	var user []*User
	var userCount int64
